...
class Detector(object):
    ...
    def run(self, params):
        ...
        self._remove_background(initial)
    ...
    def _remove_background(self, image):
        ...
        self._output['stages']['background'] = self.tobase64(edgeImg)
        # self._get_other_circles(edgeImg)
        self._clustering(blurred)


    def _clustering(self, img):
        ...
        for label in range(num_stats):
            if (stats[label, cv2.CC_STAT_WIDTH] < 20 or
                stats[label, cv2.CC_STAT_HEIGHT] < 20):
                new_image[labels == label] = 0

        self._output['stages']['clusters'] = self.tobase64(new_image)
